...
class NoImageCell: UICollectionViewCell {
    
    
    var modelItem: ModelItem! {
        didSet {
            name.text = modelItem.short_subject
            introtext.text = modelItem.introtext
            ...
            let name_size = modelItem.short_subject?.getStringSize(fontSize: 15, width: name.frame.size.width)
            name.snp.updateConstraints { (make) -> Void in
                make.height.equalTo((name_size?.height)!)
            }
            
            let introtext_size = modelItem.introtext?.getStringSize(fontSize: 14, width: introtext.frame.size.width)
            introtext.snp.updateConstraints { (make) -> Void in
                make.height.equalTo((introtext_size?.height)!)
            }
    ...
    override init(frame: CGRect) {
        ...
        introtext.snp.makeConstraints { (make) -> Void in
    ...
    lazy var introtext: UILabel = {
        ...
        introtext.numberOfLines = 2
        introtext.textAlignment = .left
        introtext.sizeToFit()
        introtext.font = UIFont.systemFont(ofSize: 14)
        introtext.textColor = UIColor.init(red: 145/255.0, green: 145/255.0, blue: 145/255.0, alpha: 1.0)
        return introtext
    }()
